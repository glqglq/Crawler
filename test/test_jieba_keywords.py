...
　　最后，如果你在使用新浪博客APP的过程中遇到任何问题或是有任何不爽，欢迎加入我们的新浪博客APP交流QQ群，群号：517353813。博客君和一众产品汪、运营喵已经在群里准备好接锅了！（手动滑稽）
    ...
    #     print  ','.join(tags),
    #     print time.time() - startstart = time.time()
    start = time.time()
    print extract_tags(content1,4),time.time() - start
    start = time.time()
    print extract_tags(content2, 4), time.time() - start
    start = time.time()
    print extract_tags(content3, 4), time.time() - start
    start = time.time()
    print extract_tags(content3, 4), time.time() - start