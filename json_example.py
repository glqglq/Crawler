...
#add_task时的ImmutableMultiDict
"""
    "start_url":{                                           #爬取目标URL
        "priority":,                                        #爬取优先级
        "type":,                                            #爬取类型，0是新闻博客，1是电商
        "alloweddomains":,                                  #允许的domains
        rules:[                                             #爬取对象的页面规则
            {                                               
                "re_url":,                                  #页面url正则
                "rule_type":,                               #0是需要翻页，1是需要提取内容+翻页
                "comment_button_type":,                     #评论按钮类型
                "comment_button_content":,                  #评论按钮内容
                "next_page_type":,                          #下一页按钮的匹配类型，0是按xpath抓，1是按class_name抓，2是按css_selector，3是按id，4是按link_text，5是按name，6是按tag_name
                "next_page_content":,                       #下一页按钮的匹配内容
                itemcontents:[                              #抓取内容映射关系
                    "key":{                                 #抓取内容的key，对应存到mongodb数据库的key
...
# add_task时的ImmutableMultiDict 2.0  即现在正在用的
"""
"XXX":{                                                  #爬取目标URL
    "priority":"",                                        #爬取优先级
    ...
    rules:[                                             #爬取对象的页面规则
        "re_url":{                                   #页面url正则                                              
            "rule_type":,                               #抓取内容的匹配类型，0是按xpath抓，1是按class_name抓，2是按css_selector，3是按id，4是按link_text，5是按name，6是按tag_name
            itemcontents:[                              #抓取内容映射关系
                "key"::"XXX"                            #抓取内容的key和匹配内容
            ]
        },
    ]
}
